...
const Create = () => {
  ...
  const openPicker = async (selectType) => {
    ...
    if (!result.canceled) {
      if (selectType === "image") {
        setForm({
          ...form, thumbnail: result.assets[0],
        });
      }
      if (selectType === "video") {
        setForm({
          ...form, video: result.assets[0],
        });
      }
    }
    // else {
  ...
  const submit = async () => {
    ...
    try {
      await createVideoPost({ ...form, userId: user.documents[0].$id });
    ...
    <SafeAreaView className="bg-primary h-full">
      <ScrollView className="px-4 my-6">
        ...
        <FormField
          title="Video Title"
          value={form.title}
          placeholder={"Craft a catchy title for your video . . ."}
